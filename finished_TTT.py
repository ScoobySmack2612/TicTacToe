...
class Application(Frame):
    def __init__(self, master):
        ...
        messagebox.showinfo("Welcome", "Presenting Tic Tac Toe")
    ...
    def pvp(self):
        self.name = simpledialog.askstring("Name", "Name of Person 1:")
        self.name2 = simpledialog.askstring("Name", "Name of Person 2:")
        self.nals = [self.name, self.name2]
        self.rand_names = self.nals[random.randrange(len(self.nals))]
        if self.rand_names == self.name:
            messagebox.showinfo("Welcome", "hello %s is 'X' and %s is 'O' 'X' goes first!" % (self.name, self.name2))
            self.playerx = self.name
            self.playero = self.name2
        else:
            messagebox.showinfo("Welcome", "hello %s is 'X' and %s is 'O' 'X' goes first!" % (self.name2, self.name))
            self.playerx = self.name2
            self.playero = self.name

        helv36 = Font(size=18)

        bops = {'width': 10, 'height': 5, 'font': helv36}
        Button(self, )
        self.square1 = Button(self, text="", command=self.square1_label, **bops)
    ...
    def pvc(self):
        ...
        self.user_name = simpledialog.askstring("Player Name", "Name of Humanoid")
        ...
        if self.rand_names == self.machine:
            ...
            messagebox.showinfo("Welcome",
                                "hello %s is 'X' and %s is 'O' 'X' goes first!" % (self.machine, self.user_name))
            self.machine_block_sequence()
        else:
            self.playerx = self.user_name
            self.playero = self.machine
            messagebox.showinfo("Welcome",
                                "hello %s is 'X' and %s is 'O'. 'X' goes first!" % (self.user_name, self.machine))
    ...
    def winner_found(self, symbol):
        ...
        if symbol == "X":
            xscore += 1
            messagebox.showinfo('Winner', symbol + ' Won!')
            answer = messagebox.askquestion('Congratulations!', 'Play Again?')
    ...
    def cats_game(self):
        messagebox.showinfo('Cats Game', 'Cats Game!')
        answer = messagebox.askquestion('Cats Game', 'Play Again?')
        if answer == "yes":
            self.resetGame(answer)
        else:
            self.master.destroy()
